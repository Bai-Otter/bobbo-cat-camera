        ...
        timeline.append(
            {
                ...
                "behaviorEvidence": _sanitize_behavior_evidence(
                    frame.get("behaviorEvidence")
                ),
    ...
    for frame in frames:
        ...
        if frame.get("eatingVerified"):
            ...
            summary["confidence"] = max(
                summary["confidence"],
                float((frame.get("behaviorEvidence") or {}).get("confidence") or 0),
            )
        elif not summary["verified"] and not summary["reason"]:
            summary["reason"] = str(
                (frame.get("behaviorEvidence") or {}).get("reason")
...
def analyze_recording(
    ...
    cute_timeline = []
    if not read_result.get("skipped") and not (read_result.get("sceneQuality") or {}).get("isDark"):
