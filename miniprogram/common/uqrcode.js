		...
		for (var i = 0; i < text.length; i++) {
			...
			if (c <= 0x7F) {
				...
				if (low >= 0xDC00 && low <= 0xDFFF) {
					var codePoint = ((high & 0x03FF) << 10 | (low & 0x03FF)) + 0x10000;
					...
					throw new Error('UTF-16 encoded string is incorrect');
				}
			} else if (c <= 0xFFFF) {
				...
				throw new Error('Character is outside the basic multilingual plane');
			}
		}
		return result;
	}
				...
				function createCanvas() {
					...
					for (var row = 0; row < qrcode.getModuleCount(); row++) {
						for (var col = 0; col < qrcode.getModuleCount(); col++) {
							var style = qrcode.isDark(row, col) ? options.foregroundColor : options
								.backgroundColor;
							...
							setTimeout(function() {
								uni.canvasToTempFilePath({
									...
									success: function(res) {
										let
											resData; // 将统一为base64格式
										let tempFilePath =
											res
											.tempFilePath; // H5为base64，其他为相对路径
										...
										if (plus) {
											...
											uni.request({
												...
												success: res => {
													resData
														=
														`data:image/png;base64,${uni.arrayBufferToBase64(res.data)}`; // 把arraybuffer转成base64
													options
														.success &&
														options
														.success(
