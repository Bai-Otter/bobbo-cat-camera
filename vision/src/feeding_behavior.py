...
BOWL_CAMERA_MIN_WIDTH_RATIO = 0.45
BOWL_CAMERA_MIN_AREA_RATIO = 0.06
BOWL_CAMERA_EDGE_MARGIN_RATIO = 0.04
BOWL_CAMERA_BOWL_GAP_RATIO = 0.08
MIN_FEEDING_FACE_WIDTH_RATIO = 0.25
MIN_FEEDING_FACE_HEIGHT_RATIO = 0.18
MIN_FEEDING_FACE_AREA_RATIO = 0.07
MIN_BOWL_CAMERA_FACE_WIDTH_RATIO = 0.08
MIN_BOWL_CAMERA_FACE_HEIGHT_RATIO = 0.05
MIN_BOWL_CAMERA_FACE_AREA_RATIO = 0.008
...
def is_bowl_camera_partial_cat(
    cat_box: dict[str, Any] | None,
    bowl_roi: dict[str, Any] | None,
    frame_shape: Sequence[int],
) -> bool:
    """Recognize a nearby cat clipped by a fixed bowl-camera composition.

    Bowl cameras frequently see a wide strip of the cat's head/back touching a
    frame edge.  Whole-animal scale rules classify that short box as distant,
    even though it overlaps the bowl in the foreground.  Require all three
    composition signals so an ordinary small corner cat is still rejected.
    """
    if not cat_box or not bowl_roi or len(frame_shape) < 2:
        return False
    frame_height = max(1.0, float(frame_shape[0]))
    frame_width = max(1.0, float(frame_shape[1]))
    x = float(cat_box.get("x") or 0)
    y = float(cat_box.get("y") or 0)
    width = max(0.0, float(cat_box.get("width") or 0))
    height = max(0.0, float(cat_box.get("height") or 0))
    if width / frame_width < BOWL_CAMERA_MIN_WIDTH_RATIO:
        return False
    if width * height / (frame_width * frame_height) < BOWL_CAMERA_MIN_AREA_RATIO:
        return False

    edge_x = frame_width * BOWL_CAMERA_EDGE_MARGIN_RATIO
    touches_side = x <= edge_x or x + width >= frame_width - edge_x
    if not touches_side:
        return False
    ...
    cat_right = x + width
    cat_bottom = y + height
    horizontal_overlap = min(cat_right, bowl_right) - max(x, bowl_left)
    if horizontal_overlap <= 0:
        return False
    vertical_gap = max(0.0, bowl_top - cat_bottom, y - bowl_bottom)
    return vertical_gap <= frame_height * BOWL_CAMERA_BOWL_GAP_RATIO


def expand_bowl_camera_contact_roi(
    bowl_roi: dict[str, Any],
    cat_boxes: Sequence[dict[str, Any]],
    frame_shape: Sequence[int],
) -> dict[str, int]:
    """Expand a small detected food patch to the visible bowl-contact band."""
    frame_height = max(1, int(frame_shape[0]))
    frame_width = max(1, int(frame_shape[1]))
    cat_bottoms = sorted(
        float(box.get("y") or 0) + max(0.0, float(box.get("height") or 0))
        for box in cat_boxes
    )
    median_cat_bottom = (
        cat_bottoms[len(cat_bottoms) // 2]
        if cat_bottoms
        else float(bowl_roi.get("y") or 0)
    )
    bowl_top = float(bowl_roi.get("y") or 0)
    bowl_bottom = bowl_top + max(0.0, float(bowl_roi.get("height") or 0))
    top = max(
        0,
        int(round(min(bowl_top, median_cat_bottom) - frame_height * 0.08)),
    )
    bottom = min(
        frame_height,
        max(
            int(round(bowl_bottom + frame_height * 0.04)),
            top + int(round(frame_height * 0.25)),
        ),
    )
    return {"x": 0, "y": top, "width": frame_width, "height": max(1, bottom - top)}
...
def is_bowl_camera_feeding_face(
    face_box: dict[str, Any] | None,
    frame_shape: Sequence[int],
) -> bool:
    if not face_box or len(frame_shape) < 2:
    ...
    return (
        face_width >= 96
        and face_height >= 96
        and face_width / frame_width >= MIN_BOWL_CAMERA_FACE_WIDTH_RATIO
        and face_height / frame_height >= MIN_BOWL_CAMERA_FACE_HEIGHT_RATIO
        and face_width * face_height / (frame_width * frame_height)
        >= MIN_BOWL_CAMERA_FACE_AREA_RATIO
    )
...
def summarize_behavior_second(
    ...
    allow_static_face_contact: bool = False,
) -> dict[str, Any]:
    ...
    static_face_contact_verified = bool(
        allow_static_face_contact and face_contact_verified
    )
    eating_verified = static_face_contact_verified or (
        motion_verified and (contact_verified or pose_ratio >= 0.5)
    )
    if mask_contact_verified:
        ...
        if static_face_contact_verified:
            confidence *= 0.82
            verification_mode = "bowl-camera-face-contact"
        else:
            verification_mode = (
...
def build_candidate_windows(
    ...
    window_start = sample_offset(coarse_anchors[0])
    window_end = window_start
    for anchor in coarse_anchors[1:]:
        anchor_offset = sample_offset(anchor)
        if anchor_offset - window_end <= MAX_FACE_RECOVERY_GAP_SECONDS:
            window_end = anchor_offset
            continue
        candidate_windows.append(
            (
                max(0.0, window_start - NEAR_FIELD_LEAD_SECONDS),
                window_end + NEAR_FIELD_TAIL_SECONDS,
            )
        )
        window_start = anchor_offset
        window_end = anchor_offset
    candidate_windows.append(
        (
            max(0.0, window_start - NEAR_FIELD_LEAD_SECONDS),
...
def expand_candidate_timeline(
    sampled_frames: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    """Add one evidence slot per second inside coarse-positive windows.

    Coarse screening is intentionally sparse. V3.2, however, consumes a
    per-second evidence timeline. Synthetic slots do not claim that a cat is
    present; they only carry the nearest coarse box as a crop hint for the
    fine face/pose verifier.
    """

    ordered = sorted(
        (dict(frame) for frame in sampled_frames),
        key=lambda frame: float(frame.get("offsetSec", frame.get("second") or 0)),
    )
    windows = build_candidate_windows(ordered)
    anchors = [
        frame for frame in ordered
        if frame.get("hasCat") or frame.get("nearBowl")
    ]
    if not windows or not anchors:
        return ordered

    existing_seconds = {int(frame.get("second") or 0) for frame in ordered}
    next_sample_index = max(
        (int(frame.get("sampleIndex", index)) for index, frame in enumerate(ordered)),
        default=-1,
    ) + 1
    expanded = list(ordered)
    for start, end in windows:
        first_second = max(0, int(math.ceil(start)))
        last_second = max(first_second, int(math.floor(end)))
        for second in range(first_second, last_second + 1):
            ...
            nearest_anchor = min(
                ...
                key=lambda frame: abs(
                    float(frame.get("offsetSec", frame.get("second") or 0)) - second
                ),
            )
            guidance_boxes = list(
                nearest_anchor.get("targetBoxes")
                or nearest_anchor.get("catBoxes")
                or []
            )
            expanded.append(
                {
                    **nearest_anchor,
                    "sampleIndex": next_sample_index,
                    ...
                    "nearBowl": False,
                    "confidence": 0.0,
                    "catBoxes": guidance_boxes,
                    "targetBoxes": guidance_boxes,
                    "coarseInterpolated": True,
                }
            )
    ...
    return sorted(
        expanded,
        key=lambda frame: (
            float(frame.get("offsetSec", frame.get("second") or 0)),
            int(frame.get("sampleIndex") or 0),
        ),
    )
...
def verify_candidate_behavior(
    ...
    candidate_windows = build_candidate_windows(ordered_frames)
    candidates = {
        sample_key(frame): frame
        for frame in ordered_frames
        if any(start <= sample_offset(frame) <= end for start, end in candidate_windows)
    }

    estimator = pose_estimator or RtmlibAnimalPoseEstimator()
    face_estimator_was_injected = face_estimator is not None
    capture = capture_factory(source_url) if capture_factory else open_video_capture(
        cv2,
        source_url,
        output_fps=max(1.0, float(sample_fps or 8.0)),
        max_width=640,
    )
    ...
    if normalized_orientation == "clockwise-90":
        frame_width, frame_height = frame_height, frame_width
    closeup_clip = False
    near_field_clip = False
    intermediate_clip = False
    bowl_camera_partial_clip = False
    verification_bowl_roi = bowl_roi
    candidate_boxes = []
    if frame_width >= 32 and frame_height >= 32:
        ...
        bowl_camera_partial_clip = any(
            is_bowl_camera_partial_cat(box, bowl_roi, frame_shape)
            for box in candidate_boxes
        )
        if bowl_camera_partial_clip:
            verification_bowl_roi = expand_bowl_camera_contact_roi(
                bowl_roi,
                candidate_boxes,
                frame_shape,
            )
    ...
    ordered_samples = sorted(
        ((sample_offset(frame), sample_key(frame)) for frame in candidates.values()),
    ...
    sample_end_offset = (sample_offsets[-1] + sample_interval) if sample_offsets else 0.0
    frame_index = 0
    ...
    try:
        while True:
            ...
            if sample_end_offset > 0 and offset_sec >= sample_end_offset:
                break
            ...
            frame_is_bowl_camera_partial = any(
                is_bowl_camera_partial_cat(box, bowl_roi, frame.shape)
                for box in cat_boxes
            ) or (not cat_boxes and bowl_camera_partial_clip)
            if frame_is_bowl_camera_partial and frame_scale == "distant":
                frame_scale = "near"
            frame_is_near_field = (
                frame_scale == "near"
                or frame_is_closeup
                or frame_is_bowl_camera_partial
            )
            ...
            if (
                ...
                if (
                    ...
                    if face and face.get("faceBox"):
                        ...
                        face_is_large_enough = is_large_feeding_face(
                            face["faceBox"], frame.shape
                        ) or (
                            frame_is_bowl_camera_partial
                            and is_bowl_camera_feeding_face(face["faceBox"], frame.shape)
                        )
                        if not face_is_large_enough:
                            ...
                            if len(landmarks) >= 48:
                                face_evidence = evaluate_face_landmark_contact(
                                    face["faceBox"], landmarks, verification_bowl_roi
                                )
                            else:
                                face_evidence = {
                                    "verified": False,
    ...
    for second, group in feeding_groups.items():
        ...
        evidence_by_second[second] = summarize_behavior_second(
            group["poses"],
            motion,
            allow_static_face_contact=bowl_camera_partial_clip,
        )
