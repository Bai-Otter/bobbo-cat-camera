...
def resolve_yolo_model_path(model_path: str | None = None) -> str:
    ...
    if is_cloud_hosting:
        requested_is_default = not requested or os.path.basename(requested) == DEFAULT_YOLO_MODEL
        if configured and os.path.isfile(configured) and requested_is_default:
            return configured
        if requested and not requested_is_default and os.path.isfile(requested):
            return requested
        attempted = configured or requested or DEFAULT_YOLO_MODEL
