...
def marker(
    ...
    if face_relation is not None:
        item["faceRelation"] = str(face_relation)
    return item
            ...
            hits.append(
                {
                    ...
                    "faceRelation": str(evidence.get("faceRelation") or "unknown"),
                }
            )
        if not hits:
            continue
        ...
        for best in sorted(selected, key=lambda item: item["offsetSec"]):
            cute_markers.append(
                marker(
                    ...
                    face_relation=best.get("faceRelation"),
                )
            )
    return cute_markers
