...
const SN_TICKET_ENDPOINT = "https://api.weixin.qq.com/wxa/getsnticket";
const DEVICE_MESSAGE_ENDPOINT = "https://api.weixin.qq.com/cgi-bin/message/device/subscribe/send";
...
function normalizeMiniProgramState(value) {
  ...
  if (state === "develop" || state === "developer") return "developer";
  if (state === "release" || state === "formal") return "formal";
  return "formal";
}
...
class WechatDeviceMessageService {
  ...
  getTemplateIds() {
    return [...new Set([this.startTemplateId, this.endTemplateId].filter(Boolean))];
  }
  ...
  getEventType(templateId) {
    if (templateId && templateId === this.endTemplateId) return "feeding_end";
    if (templateId && templateId === this.startTemplateId) return "feeding_start";
    return "";
  }
  ...
  async requestJson(url, body) {
    if (typeof this.fetchImpl !== "function") throw providerError("WECHAT_DEVICE_FETCH_UNAVAILABLE");
    ...
    try {
      ...
      if (!response || !response.ok) {
        throw providerError("WECHAT_DEVICE_PROVIDER_UNAVAILABLE", Number(response && response.status));
      }
      return await response.json();
    } catch (error) {
      if (error && error.code && String(error.code).startsWith("WECHAT_DEVICE_")) throw error;
      if (error && error.name === "AbortError") throw providerError("WECHAT_DEVICE_TIMEOUT");
      throw providerError("WECHAT_DEVICE_PROVIDER_UNAVAILABLE");
    } finally {
      clearTimeout(timer);
    }
  }
  ...
  async getAccessToken({ force = false } = {}) {
    if (!this.appId || !this.appSecret) throw providerError("WECHAT_DEVICE_NOT_CONFIGURED");
    ...
    if (Number(payload && payload.errcode)) {
      throw providerError("WECHAT_DEVICE_TOKEN_REJECTED", payload.errcode, payload.errmsg);
    }
    const value = cleanText(payload && payload.access_token, 1024);
    if (!value) throw providerError("WECHAT_DEVICE_TOKEN_INVALID_RESPONSE");
    this.cachedToken = {
      value,
  ...
  async callWithToken(endpoint, body, { retryToken = true } = {}) {
    ...
    if (providerCode && retryToken && TOKEN_ERROR_CODES.has(providerCode)) {
      ...
      return this.callWithToken(endpoint, body, { retryToken: false });
    }
    if (providerCode) {
      throw providerError("WECHAT_DEVICE_PROVIDER_REJECTED", providerCode, payload.errmsg);
    }
    return payload || {};
  }

  async getSnTicket(snValue) {
    if (!this.isConfigured()) throw providerError("WECHAT_DEVICE_NOT_CONFIGURED");
    const sn = cleanText(snValue, 128);
    if (!sn || !/^[0-9A-Za-z_-]+$/.test(sn)) throw providerError("WECHAT_DEVICE_SN_INVALID");
    const payload = await this.callWithToken(SN_TICKET_ENDPOINT, {
      sn,
      model_id: this.modelId,
    });
    const snTicket = cleanText(payload.sn_ticket, 1024);
    if (!snTicket) throw providerError("WECHAT_DEVICE_TICKET_INVALID_RESPONSE");
    return {
      sn,
      snTicket,
      modelId: this.modelId,
      tmplIds: this.getTemplateIds(),
  ...
  async send(input = {}) {
    ...
    if (!openid || !sn || !this.getTemplateIds().includes(templateId)) {
      throw providerError("WECHAT_DEVICE_SEND_INVALID");
    }
    await this.callWithToken(DEVICE_MESSAGE_ENDPOINT, {
      to_openid_list: [openid],
      sn,
      template_id: templateId,
      page: cleanText(input.page, 1024),
